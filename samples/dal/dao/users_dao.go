...
func (dao *UsersDAO) SelectUsersByIdList(id_list []int32) ([]do.UsersDO, error) {
	// TODO(@benqi): sqlmap
	// sql := fmt.Sprintf("select id, access_hash, first_name, last_name, username from users where id in (%s)", base.JoinInt32List(id_list, ","))
	//

	var sql = fmt.Sprintf("select id, access_hash, first_name, last_name, username from users where id in (?)")

	params := make(map[string]interface{})
	//
	//params["id_list1"] = base.Int32ToString(id_list[0])
	//params["id_list2"] = base.Int32ToString(id_list[1])
	//params["id_list3"] = base.Int32ToString(id_list[2])
	//glog.Info(params)
